...
def main(args=None):
    ...
    driver = uc.Chrome()
    driver.get("https://hyatt.com")
    #Once the typeahead class on location input is loaded, we've successfully bypassed the bot detection.
    WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.CSS_SELECTOR, ".typeahead")))

    inp_location = driver.find_element(By.XPATH, '//input[@name="location"]')
    inp_location.send_keys(
        "New York\n"
    ) 
    sleep(3)
